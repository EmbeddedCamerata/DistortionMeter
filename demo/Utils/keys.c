...
{
	Key[0].State = HAL_GPIO_ReadPin(Key1_GPIO_Port, Key1_Pin);  // Left
	Key[1].State = HAL_GPIO_ReadPin(Key2_GPIO_Port, Key2_Pin);  // Right
    Key[2].State = HAL_GPIO_ReadPin(KeyP_GPIO_Port, KeyP_Pin);
}
...
void Get_Key(Key_Type *Key)
{
	...
	for(i = 0; i < Key_Num; i++)
	{
		if(!Key[i].State)
		{
			...
			if(Key[i].Flag == 1 && Key[i].Count >= Key_Ok)//触发一次下降沿事件
			{
				Key[i].Flag = 2;
				Key_State[i].Fall = 1;
